...
\chapter{Empirical Results}\label{appendix:results}
...
\begin{itemize}
    \item Ping Pong --- N = 100000000 (messages sent)
    \item Thread Ring --- N = 10 (connected actors), H = 100000000 (messages sent)
    \item Count --- N = 10000000 (successive messages to be sent)
    \item Fork Join (throughput) --- N = 1000000 (messages sent to each actor), K = 10 (number of actors)
    \item Fork Join (create) --- N = 1500000 (spawned actors)
    \item Fibonacci --- N = 29 (fibonacci index)
    \item Chameneos --- N = 1200000 (number of meetings between chameneos), C = 10 (number of chameneos)
    \item Big --- N = 10 (number of actors), P = 500000 (number of pings) 
\end{itemize}
\begin{table}[H]
    \begin{center}
...
\begin{itemize}
    \item Ping Pong --- N = 10000000 (messages sent)
    \item Thread Ring --- H = 10000000 (messages sent)
    \item Count --- N = 1000000 (successive messages to be sent)
    \item Fork Join (throughput) --- K = 1 (number of actors)
    \item Fork Join (create) --- N = 200000 (spawned actors)
    \item Fibonacci --- Could not scale due as fibonacci index increases load exponentially 
    \item Chameneos --- N = 200000 (number of meetings between chameneos)
    \item Big --- P = 100000 (number of pings) 
\end{itemize}
...
For Ping Pong, 100000 messages are sent between two remote actors. This is equal to the value of the number of actors spawned in a modified version of Fork Join (create) which only remotely spawns actors without termination. This enables the reader to compare the execution time between remotely sending messages and remotely spawning actors.
\begin{table}[H]
    \begin{center}
...
The Ping Pong benchmark is executed across the different environment with an equal load of 10000000 sent messages, while the Fork Join Create spawns 1000000 actors.
\begin{table}[H]
    \begin{center}
        \begin{tabular}{|l|ll|}
        \hline
